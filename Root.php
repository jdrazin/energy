...
class Root
{
     ...
     * @throws Exception
     */
    public function interpolateAveragePowerW($entity, $type, $start, $stop): float { // returns average of points
        $points = [];
     ...
     * @throws Exception
     */
    public function skip_request($class): bool { // returns whether to skip request to prevent throttling
    ...
    public function forecast_latest($entity, $datetime): float { // returns latest fresh forecast for datetime
        $sql = 'SELECT    `w1`.`value`
                  FROM    `values` `w1`
    ...
    public function forecast_average_latest($entity, $start, $stop, $from): float { // returns average of forecast points in slot since from
