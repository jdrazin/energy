...
class Energy extends Root
{
    ...
    const           SERVER_EXTERNAL_URL_PORT = 'https://renewable-visions.tech:8445';
            ...
            if (EMAIL_ON_COMPLETION && ($email ?? false)) {
                ...
                if (filter_var($email = ($email ?? false), FILTER_VALIDATE_EMAIL) &&
                    (new SMTPEmail())->email([  'subject'   => 'Renewable Visions: your results are ready',
                                                'html'      => false,
                                                'bodyHTML'  => $error = 'Your results are ready at ' . self::SERVER_EXTERNAL_URL_PORT . '/projection.html?id=' . $projection_id . '.' . PHP_EOL . '<br>' .
                                                                        $message_setback,
