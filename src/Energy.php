...
class Energy extends Root
{
    ...
    public function slots(): string {
        $sql = 'SELECT      `n`.`start` AS `datetime`,
        ...
        if (!($stmt = $this->mysqli->prepare($sql)) ||
            !$stmt->bind_result($datetime, $total_load_kw, $previous_total_load_kw, $grid_kw, $previous_grid_kw, $solar_kw, $previous_solar_kw) ||
            !$stmt->execute()) {
        ...
        while ($stmt->fetch()) {
            $slots[] = [$datetime,  $total_load_kw,  $previous_total_load_kw, $grid_kw, $previous_grid_kw,   $solar_kw, $previous_solar_kw];
        }
