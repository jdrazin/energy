...
class Energy extends Root
{
    ...
    public function slots(): string {
        ...
        if (!($stmt = $this->mysqli->prepare($sql)) ||
            !$stmt->bind_result($slot, $datetime, $total_load_kw, $previous_total_load_kw, $grid_kw, $previous_grid_kw, $solar_kw, $previous_solar_kw) ||
            !$stmt->execute()) {
        ...
        $slots = [];
        $slots[]     = ['slot', 'datetime', 'total_load_kw', 'previous_load_kw',     'grid_kw', 'previous_grid_kw', 'solar_kw', 'previous_solar_kw'];
        while ($stmt->fetch()) {
            $slots[] = [$slot,  $datetime,  $total_load_kw,  $previous_total_load_kw, $grid_kw, $previous_grid_kw,   $solar_kw, $previous_solar_kw];
        }
