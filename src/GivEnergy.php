...
class GivEnergy extends Root
{
     ...
     * @throws Exception
     */
    public function batteryLevelSlotBeginExtrapolateKwh($timestamp_start): float
    {
        //
        ...
        if (GIVENERGY_ENABLE) {
            switch ($mode) {
                ...
                case 'DISCHARGE':
                {
                    ...
                    $eco_mode = ($mode == 'DISCHARGE') ? 1 : 0; // enable ECO mode if discharging to allow battery to continue to handle load when target met (otherwise peak rate incurred)
