...
class HotWaterTank extends ThermalTank
{
    ...
    public float $target_temperature_c, $immersion_w, $one_way_storage_efficiency;
    ...
    public function __construct($check, $config, $component_name, $time)
    {
        if ($this->include = $check->checkValue($config, self::COMPONENT_NAME, [], 'include', self::CHECKS, true)) {
            ...
            $this->thermal_compliance_heating_c_per_j           = 1.0 / ($volume_m3 * self::HEAT_CAPACITY_WATER_J_PER_M3_K);
            ...
            $this->thermal_compliance_heating_c_per_j           = $this->thermal_compliance_heating_c_per_j * $this->one_way_storage_efficiency;
            $this->thermal_compliance_cooling_c_per_j                    = $this->thermal_compliance_heating_c_per_j;
        }
    }
}