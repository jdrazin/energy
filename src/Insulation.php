...
class Insulation extends Component
{
    ...
    const array CHECKS = [
        ...
        'cost'                                   => ['array'   => null      ],
        'energy_saving_percent' => ['range'   => 0.0, 100.0]
    ];
    ...
    public function __construct($check, $config, $time)
    {
        if ($this->include = $check->checkValue($config, self::COMPONENT_NAME, [], 'include', self::CHECKS, true)) {
            ...
            $energy_saving_percent = $check->checkValue($config, self::COMPONENT_NAME, [], 'energy_saving_percent', self::CHECKS);
            $this->space_heating_demand_factor = (100.0 - $energy_saving_percent)/100.0;
        }
    }
}