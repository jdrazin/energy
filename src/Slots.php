...
class Slots
{
    ...
    public function __construct($datetime_earliest, $until)
    {
        ...
        $day_slot = (int)($day_minutes / DbSlots::SLOT_DURATION_MIN);
        if ($day_minutes % DbSlots::SLOT_DURATION_MIN) {
            $day_slot++;
        }
        $datetime_earliest->setTime(0, 0);
        $datetime_earliest->modify('+' . DbSlots::SLOT_DURATION_MIN * $day_slot . ' minute');
    ...
    public function next_slot(): ?array
    {
        ...
        $this->datetime->modify('+' . DbSlots::SLOT_DURATION_MIN / 2 . ' minute');
